...
def split_keep_delimiter(sentence: str, delimiter: str):
...
# Modified tokenization to separately tokenize words in csv lists
#   ex. in "word1,word2,word3", word1, word2, word3, and the commas will be tokenized separately and then concatenated
def sent_encode(tokenizer, sent):
    ...
    if sent == "":
        ...
        if version.parse(trans_version) >= version.parse("4.0.0"):
            return [tokenizer.cls_token_id] + list(chain.from_iterable([tokenizer.encode(
                val,
                add_special_tokens=False,
                add_prefix_space=True,
                max_length=tokenizer.model_max_length,
                truncation=True,
            ) for val in split_keep_delimiter(sent, ",")])) + [tokenizer.sep_token_id]
        ...
        if version.parse(trans_version) >= version.parse("4.0.0"):
            tok = list(chain.from_iterable([tokenizer.encode(
                val,
                add_special_tokens=False,
                max_length=tokenizer.model_max_length,
                truncation=True,
            ) for val in split_keep_delimiter(sent, ",")]))
            return [tokenizer.cls_token_id] + list(chain.from_iterable([tokenizer.encode(
                val,
                add_special_tokens=False,
                max_length=tokenizer.model_max_length,
            ...
            return [tokenizer.cls_token_id] + list(chain.from_iterable([tokenizer.encode(
                val, add_special_tokens=False, max_length=tokenizer.max_len
            ) for val in split_keep_delimiter(sent, ",")])) + [tokenizer.sep_token_id]
        else:
            raise NotImplementedError(
                f"transformers version {trans_version} is not supported"
